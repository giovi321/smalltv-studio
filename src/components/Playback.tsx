...
export function Playback() {
  const playing = useStudio(s => s.playing), elapsed = useStudio(s => s.elapsed), baseTime = useStudio(s => s.baseTime);
  return (
    <section className="playback" aria-label="Playback">
      <div className="playback-row">
        ...
        <label className="time-label">Simulated time
          <input
            type="datetime-local" step={1} defaultValue={toLocalInput(baseTime)}
            onChange={e => {
